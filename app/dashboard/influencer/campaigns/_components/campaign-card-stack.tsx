...
export function CampaignCardStack({ campaigns, className }: Props) {
  ...
  useEffect(() => {
    ...
    const updateViewport = () => {
      const { width: widthPx, height: heightPx } = node.getBoundingClientRect();
      if (widthPx <= 0 || heightPx <= 0) return;

      const peekOverhead = (VISIBLE_COUNT - 1) * OFFSET_Y;
      const availableHeight = Math.max(heightPx - 2, 0);

      let cardWidth = widthPx;
      let cardHeight = cardWidth / CARD_ASPECT_RATIO;
      if (cardHeight + peekOverhead > availableHeight) {
        cardHeight = availableHeight - peekOverhead;
        cardWidth = cardHeight * CARD_ASPECT_RATIO;
      }

      setCardViewport({
        width: Math.max(Math.min(cardWidth, widthPx), 0),
        height: Math.max(cardHeight, 0),
      });
    };
  ...
  const visibleCards = Array.from(
    ...
    (_, index) => campaigns[(safeIndex + index) % total],
  );
          ...
          >
            <AnimatePresence>
              {visibleCards.map((card, index) => {
                ...
                const brightness = Math.max(0.5, 1 - index * DIM_STEP);
                ...
                return (
                  <m.li
                    ...
                    animate={{
                      ...
                      filter: `brightness(${brightness}) saturate(${1 - index * 0.08})`,
                      zIndex: VISIBLE_COUNT - index,
                      opacity: dragDir && isFront ? 0 : 1 - index * 0.08,
                    }}
                    exit={{
                      opacity: 0,
                      scale: 0.88,
