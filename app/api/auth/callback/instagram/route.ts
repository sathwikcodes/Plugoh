...
function errorRedirect(request: NextRequest, reason: string) {
  return NextResponse.redirect(
    new URL(`/onboarding?error=${reason}`, request.url),
  );
}
...
export async function GET(request: NextRequest) {
  ...
  const [role, , userId] = state.split(":") as [AppRole?, string?, string?];
  if (!userId || (role !== "influencer" && role !== "business")) {
    return errorRedirect(request, "invalid_state");
  }
  ...
  try {
    ...
    if (role === "influencer") {
      ...
      const { data: existingProfile } = await db
        ...
        .select("brand_name,brand_summary")
        .eq("user_id", userId)
        .maybeSingle();

      const { error: upsertError } = await db.from("business_profiles").upsert(
        {
          user_id: userId,
          ig_user_id: igUserId,
          ig_username: profile.username,
