...
gstack is installed at `~/.Codex/skills/gstack` and provides eight cognitive specialist skills. **Use the right skill for each phase of work — do not default to the generic assistant when a specialist applies.**
...
The browse binary is at `~/.Codex/skills/gstack/browse/dist/browse`. First run starts Chromium (~3s); subsequent commands are ~100ms. State (cookies, tabs, login session) persists between commands.
